...
const AllBrand = () => {
  ...
  const handleBrandDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(laravel_base_url + `brand/${id}`)
          .then((res) => {
            getBrands();
            Swal.fire({
              position: "top-end",
  ...
  return (
    <>
      <PageMeta
        title="All Brand | Dashboard"
        description="This is all brand of inventaix company"
      />
      <PageBreadcrumb pageTitle={"All Brand"} />
                ...
                {Object.keys(brands).length > 0
                  ? brands.map((brand, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">{brand.created_by}</td>
                        <td className="py-3 px-4">
                          {brand.updated_at === "Not Updated Yet"
                            ? brand.created_at
                            : brand.updated_at}
                        </td>
                        <td className="py-3 px-4">
